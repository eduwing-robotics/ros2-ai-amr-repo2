...
namespace Unityctl.Plugin.Editor.Commands
{
    public class ScreenshotHandler : CommandHandlerBase
    {
        ...
        protected override CommandResponse ExecuteInEditor(CommandRequest request)
        {
            ...
            // URHYNIX 패치: view=game일 때 overlay UI 기본 포함 (UI Toolkit ScreenSpaceOverlay 캡처).
            var includeOverlayUi = request.GetParam<bool>("includeOverlayUi",
                string.Equals(request.GetParam("view", "scene"), "game", StringComparison.OrdinalIgnoreCase));
