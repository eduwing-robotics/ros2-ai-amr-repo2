#!/usr/bin/env python3
"""Rotate Gen.G until its docking ArUco marker is centered.

This controller is configured exclusively for the Gen.G camera and marker.
"""
...
class ArucoAligner(Node):
    def __init__(
        ...
        *,
        ...
        super().__init__("aruco_align_geng_bearing")
    ...
    def on_camera_info(self, message: CameraInfo) -> None:
        ...
        # Gen.G camera_ros often publishes an empty calibration (all zeros).
    ...
    def on_image(self, message: CompressedImage) -> None:
        ...
        for index, detected_id in enumerate(ids.flatten()):
            ...
            if self.use_image_error or self.camera_matrix is None:
                # Prefer image-center error for Gen.G (empty camera_info).
                fx = width / (2.0 * math.tan(math.radians(31.1)))
                self.last_bearing = math.atan2(marker_cx - (width * 0.5), fx)
                return
...
def main() -> int:
    ...
    parser.add_argument("--marker-id", type=int, default=12)
    parser.add_argument("--marker-size", type=float, default=0.05)
    parser.add_argument("--cmd-topic", default="/cmd_vel")
    parser.add_argument("--image-topic", default="/tb3_2/camera/image_raw/compressed")
    parser.add_argument("--camera-info-topic", default="/tb3_2/camera/camera_info")
