#!/usr/bin/env bash
# Gen.G Nav2: Gen.G RPP + raw /scan + raw odom TF (match 검증된 path).
# Footprint only difference from 기준 body size: 30x20 cm.
set -euo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
export ROBOT="${ROBOT:-geng}"
export NAV2_PARAMS="${ROOT}/config/nav2_params_geng_rpp.yaml"
...
PARAMS_RUNTIME="${ROOT}/.runtime_nav2_params_geng_rpp.yaml"
sed \
