#!/usr/bin/env bash
# T1-only Nav2 foreground launcher.
set -euo pipefail
...
export NAV2_PARAMS="${NAV2_PARAMS:-${ROOT}/config/nav2_params_t1_patrol_rpp.yaml}"
...
if [[ "${START_BRINGUP}" == "1" ]]; then
  if [[ -x "${ROOT}/scripts/_ensure_robot_ready.sh" ]]; then
    bash "${ROOT}/scripts/_ensure_robot_ready.sh" 90
  else
    echo "[WARN] START_BRINGUP=1 이지만 _ensure_robot_ready.sh 없음 — 로봇 bringup은 직접 확인하세요"
  fi
fi
