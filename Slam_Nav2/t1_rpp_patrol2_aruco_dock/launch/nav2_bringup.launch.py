"""T1 Nav2 bringup — standalone (no museum_nav_bringup install required)."""
    ...
    return LaunchDescription(
        [
            DeclareLaunchArgument(
                "map",
                default_value=os.path.join(_PKG_ROOT, "maps", "t1_map_new.yaml"),
                description="Path to map yaml",
            ),
            DeclareLaunchArgument(
                "params_file",
                default_value=os.path.join(
                    _PKG_ROOT, "config", "nav2_params_t1_patrol_rpp.yaml"
                ),
