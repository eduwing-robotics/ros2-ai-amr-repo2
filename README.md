...
- **운영 대상**: T1(<code>tb3_1</code>)과 Gen.G(<code>tb3_2</code>) TurtleBot3 Burger 2대
...
- **T1**
  - **ROS namespace**: <code>tb3_1</code>
  - **ROS domain**: <code>2</code>
  - **카메라·센서**: RealSense D435
  - **주요 역할**: 비전, 자율주행, ArUco 정렬·도킹
- **Gen.G**
  - **ROS namespace**: <code>tb3_2</code>
  - **ROS domain**: <code>1</code>
  - **카메라·센서**: Pi Camera, Arduino 환경 센서
  - **주요 역할**: 순찰, 환경 감지, ArUco 정렬·도킹
### 명령과 데이터 흐름

- **Web Dashboard ↔ Robot**
  - **인터페이스**: HTTP API, <code>rclpy</code>, ROS topic/action, SSH
  - **역할**: 설정, 카메라·센서 구독, 수동 주행, Nav2 목표, bring-up
- **Unity Dashboard → Robot**
  - **인터페이스**: <code>/&lt;robot&gt;/prepare_drive</code>, <code>/&lt;robot&gt;/patrol_waypoints</code>, <code>/&lt;robot&gt;/goal_pose</code>
  - **역할**: 주행 준비, 순찰 경로, 단발 출동 목표 전달
- **Robot → Unity Dashboard**
  - **인터페이스**: <code>/&lt;robot&gt;/pose</code>, camera/LiDAR/sensor topics
  - **역할**: 지도 기준 위치, 영상, 스캔, 환경 센서 상태 전달
- **Unity Dashboard → Supabase**
  - **인터페이스**: session/log/dispatch/pose writes
  - **역할**: 시연과 운영 결과 저장
...
- **로봇 하드웨어**
  - **구성**: TurtleBot3 Burger, Raspberry Pi, OpenCR, LDS LiDAR, Dynamixel
  - **역할**: 차동구동, LiDAR 취득, odometry
- **카메라**
  - **구성**: RealSense D435, Pi Camera v2
  - **역할**: RGB/RGB-D 스트림, 객체 탐지, 마커 인식
- **환경 센서**
  - **구성**: Arduino Uno, PIR, 소리, 온도, 거리 센서
  - **역할**: 박물관 상황 이벤트 수집
- **운영 데이터**
  - **구성**: Supabase, PostgreSQL, JSONL run logs
  - **역할**: 세션·이벤트·출동·pose·주행 결과 기록
- **3D 제작·검증**
  - **구성**: Arduino 배선 참고 자료, 로봇 부속 3D 설계·출력
  - **역할**: 실제 센서와 로봇 부속품 연결 검증
...
- **위치 추정**: 저장된 <code>PGM/YAML</code> 지도에서 AMCL이 LiDAR scan, wheel odometry, TF를 결합해 <code>map</code> 기준 pose를 추정합니다. 시작 시 <code>/initialpose</code> 재시딩으로 수렴을 돕습니다.
...
- **Dual live camera**
  - **입력과 방법**: D435·IMX219 compressed topic, <code>image_transport</code>, JPEG, ROS-TCP subscriber, MJPEG
  - **결과**: 두 로봇의 영상과 FPS를 Web·Unity 관제 화면에 표시
- **박물관 객체 감지**
  - **입력과 방법**: Ultralytics YOLO, person 보조 모델, class-aware NMS, 연속 프레임 확인, CLAHE·sharpen
  - **결과**: 사람·화재·연기·조각상 overlay와 감지 상태
- **ArUco 정렬**
  - **입력과 방법**: <code>DICT_4X4_50</code>, marker ID, IPPE square <code>solvePnP</code>, image-center bearing fallback
  - **결과**: 목표 bearing 오차와 회전 명령
- **후방 도킹**
  - **입력과 방법**: 후방 LiDAR 점군의 RANSAC wall fit, 거리·각도 폐루프
  - **결과**: 벽 기준 거리 유지 후진 도킹
- **그림 진위 판별**
  - **입력과 방법**: 고정 pose에서 자른 Bacchus ROI, ImageNet pretrained EfficientNet-B0, 224×224
  - **결과**: <code>GENUINE</code> / <code>FAKE</code> / <code>RECHECK</code>
- **3D 재구성**
  - **입력과 방법**: RealSense RGB-D rosbag, RTAB-Map, crop·outlier filtering, PLY/PCX import
  - **결과**: Unity Dashboard에서 확인 가능한 3D 점군
...
- **핵심 역할**
  - **TurtleBot Web Dashboard**: 실제 TurtleBot의 설정·진단·지도·주행 운영
  - **Unity Dashboard**: 다중 로봇 디지털 트윈·상황 관제·기록
- **연결 방식**
  - **TurtleBot Web Dashboard**: Browser ↔ HTTP API ↔ <code>rclpy</code> ↔ ROS 2
  - **Unity Dashboard**: Unity ↔ ROS-TCP Endpoint ↔ ROS 2
- **지도**
  - **TurtleBot Web Dashboard**: 저장 지도 선택, 벽·장애물 편집, 새 점유지도 제작
  - **Unity Dashboard**: 2D·2.5D·3D 지도와 로봇 위치·경로 시각화
- **주행**
  - **TurtleBot Web Dashboard**: 수동 조작, 목표·경유지, A* 경로, 반복 주행
  - **Unity Dashboard**: 주행 준비, 순찰 waypoint, 단발 출동, 정지·복귀
- **센서**
  - **TurtleBot Web Dashboard**: LiDAR 안전 반경, odometry, raw/compressed 카메라
  - **Unity Dashboard**: 카메라, LiDAR, 환경 센서, 상태·이벤트 패널
- **운영 강점**
  - **TurtleBot Web Dashboard**: SSH bring-up, OpenCR 확인, 현장 진단
  - **Unity Dashboard**: 시나리오 재현, 공간 상황 이해, 운영 이력
- **소스**
  - **TurtleBot Web Dashboard**: <a href="https://github.com/ensacom2019/TurtleBot_Dashboard">TurtleBot_Dashboard</a>
  - **Unity Dashboard**: 이 저장소의 <a href="./UNITY/"><code>UNITY/</code></a>
...
- **관제·시각화**: Unity 6.3 LTS, C#, ROS-TCP-Connector, Python, <code>rclpy</code>, HTTP, MJPEG
...
- **YOLO 감지·그림 진위 판별**: <a href="./Vision_AI/README.md">Vision_AI/README.md</a>
- **T1·Gen.G 순찰·도킹**: <a href="./Slam_Nav2/README.md">Slam_Nav2/README.md</a>
- **로봇 사양과 통합 메모**: <a href="./Vision_AI/ROBOT_SPECS.md">Vision_AI/ROBOT_SPECS.md</a> · <a href="./Vision_AI/docs/team-integration.md">team-integration.md</a>
- **Web Dashboard**: <a href="https://github.com/ensacom2019/TurtleBot_Dashboard">ensacom2019/TurtleBot_Dashboard</a>
