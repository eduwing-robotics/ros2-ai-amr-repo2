...
namespace Unity.Robotics.ROSTCPConnector
{
    public class ROSConnection : MonoBehaviour
    {
        ...
        {
            // URHYNIX 2026-06-29: 멀티 ROSConnection 인스턴스(로봇별 endpoint) 지원 — 공유 static 버퍼
            //   s_FourBytes/s_TopicScratchSpace를 호출별 로컬 버퍼로 교체. 두 reader 스레드가 같은 static
            //   버퍼를 동시에 써서 메시지 파싱이 깨지던("No more data available" 연결 끊김) race를 제거.
            ...
            byte[] topicScratch = new byte[topicLength];
            // Read and convert topic name
            await ReadToByteArray(networkStream, topicScratch, topicLength, sleepMilliseconds, token);
            string topicName = Encoding.ASCII.GetString(topicScratch, 0, topicLength);
