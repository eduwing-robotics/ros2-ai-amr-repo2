...
namespace Unityctl.Plugin.Editor.Commands
{
    ...
    public sealed class DescribeTypeHandler : CommandHandlerBase
    {
        ...
        protected override CommandResponse ExecuteInEditor(CommandRequest request)
        {
            ...
            // GetParam<T>는 where T:struct 제약이라 int? 직접 불가(CS0453). -1 sentinel로 받아 nullable 복원.
            int maxMembersRaw = request.GetParam<int>("maxMembers", -1);
            ...
            if (full)
            {
                ...
                // List<JObject>는 JToken 인덱서에 직접 대입 불가(CS0029). JArray로 감싼다.
