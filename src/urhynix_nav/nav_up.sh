...
echo "== arena_shared 맵 scp(→ ~/maps/arena_shared/, yaml의 image:arena_shared.pgm 참조 유지) =="
$SSH "$GENJI" 'mkdir -p ~/maps/arena_shared'
scp -o ConnectTimeout=10 "$MAPDIR/arena_shared.pgm"  "$GENJI":'~/maps/arena_shared/' || { echo "맵 pgm scp 실패"; exit 1; }
scp -o ConnectTimeout=10 "$MAPDIR/arena_shared.yaml" "$GENJI":'~/maps/arena_shared/' || { echo "맵 yaml scp 실패"; exit 1; }

echo "== 젠지(tb3_2) AMCL+Nav2 기동 + endpoint =="
$SSH "$GENJI" "bash /tmp/_robot_nav_up.sh tb3_2 $IX $IY $IYAW yes \$HOME/maps/arena_shared/arena_shared.yaml"
