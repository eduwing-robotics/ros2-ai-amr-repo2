...
> **readyd (2026-07-09 준비, 미설치)**: `readyd.py` + `urhynix-readyd.service` — "주행준비 원버튼"
> 온디맨드 트리거. Unity `/tb3_1/prepare_drive`(Bool) 수신 → `~/t1_drive_ready.sh` 1회 실행 →
> `/tb3_1/drive_ready_status`(String) 진행 회신. **bringup을 상주로 만들지 않음**(버튼 시 1회 실행)이라
> 위 "수동 lifecycle 보존" 원칙과 충돌 안 함. 배포:
> `scp readyd.py t1:~/ && scp urhynix-readyd.service t1:~/.config/systemd/user/ && ssh t1 'systemctl --user daemon-reload && systemctl --user enable --now urhynix-readyd'`
