...
| `/<robotId>/goal_pose` | `geometry_msgs/PoseStamped` | Unity ControlRoom `DispatchPublisher` | `patrol_waypoints_bridge.py` | 단발 | 맵 우클릭 단일 출동 목표(frame_id=map). 로봇별 네임스페이스로 선택 로봇만 수신하고 브리지가 Nav2 goToPose로 수행. |
